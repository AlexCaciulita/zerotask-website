...
export default function LandingPage() {
  ...
  const [appName, setAppName] = useState('Wit Dating');
  const [tagline, setTagline] = useState('Never Run Out of Things to Say on Dating Apps');
  const [description, setDescription] = useState('AI-powered message assistant for Hinge, Tinder & Bumble. Write better messages, get more replies.');
  const [ctaText, setCtaText] = useState('Download Free');
  const [appStoreUrl, setAppStoreUrl] = useState('https://apps.apple.com/app/witdating');
  const [playStoreUrl, setPlayStoreUrl] = useState('https://play.google.com/store/apps/details?id=com.witdating');
  const [accentColor, setAccentColor] = useState('#10b981');
  const [showPrivacy, setShowPrivacy] = useState(false);
  const [hostOnZeroTask, setHostOnZeroTask] = useState(false);
  const [subdomain, setSubdomain] = useState('witdating');
  ...
  const [features, setFeatures] = useState([
    { name: 'AI Message Writer', desc: 'Craft the perfect opener for any match' },
    { name: 'Conversation Coach', desc: 'Never run out of things to say' },
    { name: 'Profile Optimizer', desc: 'AI-powered bio suggestions' },
    { name: 'Response Predictor', desc: 'Know which messages get replies' },
  ]);
