...
const defaultFeedItems: FeedItem[] = [
  {
    id: '1', type: 'keyword', title: '\u201cdating message helper\u201d trending',
  ...
  {
    id: '2', type: 'competitor', title: 'Rizz App updated their listing',
    summary: 'Changed subtitle to \u201cAI Dating Assistant & Message Writer\u201d — targeting your keywords.',
    detail: 'Rizz App updated their App Store listing 3 hours ago. They added 2 keywords you\'re tracking: "dating assistant" and "message writer".',
  ...
  },
  {
    ...
    summary: '3 variations generated targeting your top keyword cluster around "dating app opener". CTR prediction: 4.2%.',
    detail: 'Generated 3 headline variations optimized for your top keyword cluster: dating openers, conversation starters, and Hinge prompts.',
...
function useAppStoreAnalytics() {
  ...
  const fetchAnalytics = useCallback(async (force = false) => {
    ...
    try {
      // Check if configured
      const statusRes = await fetch('/api/analytics', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'status' }) });
      if (!statusRes.ok) { setConnected(false); return null; }
      
      // Fetch apps
      const appsRes = await fetch('/api/analytics', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'apps' }) });
      ...
      if (apps.length > 0) {
        ...
        try {
          const reviewsRes = await fetch('/api/analytics', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'reviews', appId: apps[0].id }) });
