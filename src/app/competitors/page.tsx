...
const DEFAULT_COMPETITORS: CompetitorData[] = [
  {
    id: 'hinge', name: 'Hinge', icon: '🌹', rating: 4.5, reviews: 890000, downloads: '100M+',
    keywordCount: 85, sharedKeywords: 28, uniqueKeywords: 57, yourUnique: 22, alerts: true,
    timeline: [
      { date: 'Feb 1, 2026', event: 'Added "AI message features" to app title', impact: '+8 positions on 3 shared keywords' },
      { date: 'Jan 15, 2026', event: 'Updated screenshots with AI features', impact: '+12% conversion rate estimated' },
    ],
    strategy: 'Hinge added \'AI message features\' to title on Feb 1. They gained 8 positions on 3 shared keywords. Recommended counter: add \'AI message\' to your subtitle.',
  },
  {
    id: 'bumble', name: 'Bumble', icon: '🐝', rating: 4.0, reviews: 420000, downloads: '50M+',
    keywordCount: 72, sharedKeywords: 22, uniqueKeywords: 50, yourUnique: 28, alerts: false,
    timeline: [
      { date: 'Jan 28, 2026', event: 'Launched AI conversation starters', impact: 'Targeting "ai dating" keyword cluster' },
    ],
    strategy: 'Bumble launched AI conversation starters targeting your core keyword cluster. Recommended: highlight unique AI features like personalized message crafting that Bumble lacks.',
  },
  {
    id: 'tinder', name: 'Tinder', icon: '🔥', rating: 4.6, reviews: 1200000, downloads: '200M+',
    keywordCount: 110, sharedKeywords: 35, uniqueKeywords: 75, yourUnique: 15, alerts: false,
    timeline: [
      { date: 'Feb 5, 2026', event: 'Added AI profile suggestions', impact: 'New feature targeting casual daters' },
    ],
    strategy: 'Tinder is going all-in on AI features. Recommended: position Wit Dating as the "meaningful connections" app — own the quality-over-quantity angle.',
  },
];
...
export default function CompetitorsPage() {
  ...
  const [alerts, setAlerts] = useState<Record<string, boolean>>({ hinge: true, bumble: false, tinder: false });
  ...
  const analyzeCompetitor = useCallback(async (competitor: CompetitorData) => {
    ...
    try {
      ...
      const res = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ task: 'analyze-competitor', appData, competitor: { name: competitor.name, description: newDescription || undefined } }) });
      const data = await res.json();
  ...
  const addCompetitor = useCallback(async () => {
    ...
    try {
      const scrapeRes = await fetch('/api/scrape/competitor', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: newName }) });
    ...
    try {
      const appData = { name: app?.name || 'App', category: app?.category || 'General', platform: app?.platform?.toLowerCase() || 'ios' };
      const res = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ task: 'analyze-competitor', appData, competitor: { name: scrapedComp.name, description: scrapedDescription || undefined } }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Analysis failed');
      if (data.result) {
          ...
          <div className="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-neutral-900 rounded-xl shadow-2xl border border-neutral-200 dark:border-neutral-800 p-6 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. Hinge"
