...
import { Shield, Bell, BellOff, TrendingUp, AlertTriangle, Eye, Star, Download, Clock, Zap, ChevronRight, Plus, X, Loader2, Sparkles, BarChart3 } from 'lucide-react';
import clsx from 'clsx';
import { useActiveApp } from '@/lib/useActiveApp';
import { apiFetch } from '@/lib/api-client';
import EmptyState from '@/components/EmptyState';
import { getCompetitors, saveCompetitor, deleteCompetitor } from '@/lib/db';
...
export default function CompetitorsPage() {
  ...
  useEffect(() => {
    async function loadCompetitors() {
      ...
      if (data && data.length > 0) {
        // Map DB data to the page's competitor format
        const mapped: CompetitorData[] = data.map((c: Record<string, unknown>) => ({
          id: c.id as string,
          name: c.name as string,
          icon: (c.icon as string) || '?',
          rating: (c.rating as number) || 0,
          reviews: (c.review_count as number) || 0,
          downloads: (c.downloads_estimate as string) || '0',
          keywordCount: (c.keywords_total as number) || 0,
          sharedKeywords: (c.keywords_shared as number) || 0,
          uniqueKeywords: (c.keywords_unique as number) || 0,
          yourUnique: 0,
          alerts: false,
          timeline: [],
          strategy: '',
    ...
    loadCompetitors();
    // Also load cached analyses from localStorage
    try {
  ...
  const addCompetitor = useCallback(async () => {
    ...
    try {
      ...
      if (scrapeData.results && scrapeData.results.length > 0) {
        const scrapedApp = scrapeData.results[0];
        scrapedComp = { ...newComp, name: scrapedApp.name || newComp.name, rating: scrapedApp.rating || 0, reviews: scrapedApp.reviewCount || 0, downloads: scrapedApp.reviewCount > 100000 ? `${Math.round(scrapedApp.reviewCount / 100000) * 10}M+` : scrapedApp.reviewCount > 1000 ? `${Math.round(scrapedApp.reviewCount / 1000)}K+` : 'Unknown', keywordCount: scrapedApp.keywords?.length || 0, strategy: 'Analyzing with real data...' };
        scrapedDescription = scrapedDescription || `${scrapedApp.description?.slice(0, 500) || ''} Category: ${scrapedApp.category || ''}. Rating: ${scrapedApp.rating}. Reviews: ${scrapedApp.reviewCount}.`;
    ...
    try {
      ...
      if (data.result) {
        ...
        // Persist to Supabase with all scraped data
        saveCompetitor({
          app_id: id,
          name: scrapedComp.name,
          icon: scrapedComp.icon,
          rating: scrapedComp.rating,
          review_count: scrapedComp.reviews,
          downloads_estimate: scrapedComp.downloads,
          keywords_total: scrapedComp.keywordCount,
          keywords_shared: scrapedComp.sharedKeywords,
          keywords_unique: scrapedComp.uniqueKeywords,
          scraped_data: data.result,
        }).catch(() => {});
      }
    } catch (err: unknown) { setAnalysisError(err instanceof Error ? err.message : 'Analysis failed'); }
  ...
  const analysis = selected ? aiAnalyses[selected.id] : undefined;
    ...
    <div className="space-y-6">
      ...
      {competitors.length === 0 && !analyzing && (
        <EmptyState
          ...
          description="Search for competitor apps to track their keywords, ratings, and strategy changes."
        />
      )}

      {/* ── Competitor Cards & Details (only when data exists) ── */}
      {competitors.length > 0 && selected && (
        <>
